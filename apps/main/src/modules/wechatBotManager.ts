...
export class WechatBotManager {
  ...
  async stop(): Promise<void> {
    ...
    if (this.pollPromise) {
      await Promise.race([this.pollPromise, new Promise((resolve) => setTimeout(resolve, 2000))])
    }
    this.getUpdatesBuf = ''
  ...
  private getRecentSessions(limit = 20): WechatBotSessionRecord[] {
    return [...this.recordMap.values()].sort((a, b) => b.lastUsedAt - a.lastUsedAt).slice(0, limit)
  }

  private async waitForScan(qrcode: string): Promise<void> {
    if (!qrcode) return

    while (this.qrcode === qrcode && !this.connector.authenticated) {
      try {
        const status = await this.connector.checkAuthStatus(qrcode)
  ...
  private async handleCommand(text: string): Promise<WechatCommand> {
    ...
    const found = [...this.recordMap.values()].find((record) =>
      record.sessionId.startsWith(targetName)
    )
    if (!found) {
      return {
        kind: 'reply',
  ...
  private buildRecentSessionsText(): string {
    ...
    records.forEach((record, index) => {
      const active = record.sessionId === this.activeSessionId ? ' ◀ 当前' : ''
      const short = record.sessionId.slice(0, 8)
      lines.push(
        `${index + 1}. \`${short}\`${active}  最近使用 ${formatRelative(record.lastUsedAt)}`
      )
    })
  ...
  private async createAgentSession(): Promise<string> {
    if (this.recordMap.size >= MAX_WECHAT_SESSIONS) {
      ...
      if (oldest) {
        this.recordMap.delete(oldest.sessionId)
      }
    }
    ...
    const sessionId = await this.appManager.agentManager.createSession({
      workspacePath: null,
  ...
  private async getOrCreateActiveSessionId(): Promise<string> {
    if (this.activeSessionId && this.appManager.agentManager.hasSession(this.activeSessionId)) {
      const record = this.recordMap.get(this.activeSessionId)
      if (!record) {
