...
export const useWorkspaceExplorerActions = () =>
  useWorkspaceExplorerStore((state) => state.actions)
...
function isDeletedByEvent(target: string[], event: WorkspaceFileChangedEvent) {
  if (event.event === 'unlink') return sameTarget(target, event.target)
  if (event.event === 'unlinkDir') return isTargetInside(event.target, target)
  return false
}

function isTargetInside(parent: string[], target: string[]) {
  return (
    target.length >= parent.length &&
    parent.every((segment, index) => target[index] === segment)
  )
}
...
function get() {
  return useWorkspaceExplorerStore.getState()
}

function set(recipe: (state: Draft<WorkspaceExplorerStore>) => void) {
  useWorkspaceExplorerStore.setState(produce(recipe))
}
