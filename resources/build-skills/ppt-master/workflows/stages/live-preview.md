...
```bash
python ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --daemon
```
...
```bash
python ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --daemon --no-browser
# or for Step 6's auto-startup on a remote host:
python ${SKILL_DIR}/scripts/svg_editor/server.py <project_path> --live --daemon --no-browser
```
